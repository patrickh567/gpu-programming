...
void blurWithCuda(unsigned char* h_blurred_image, const unsigned char* h_input_image, 
    ...
    unsigned char* d_blur_fliter;
    const unsigned int image_size = x_cols * y_rows * sizeof(char);
    ...
    checkCuda(cudaMalloc((void**)&d_blur_fliter, BLUR_FILTER_WIDTH * BLUR_FILTER_WIDTH));

    // Copy input vectors from host memory to GPU buffers.
    checkCuda(cudaMemcpy(d_input_image, h_input_image, image_size, cudaMemcpyHostToDevice));
    checkCuda(cudaMemcpy(d_blur_filter, &BLUR_FILT, BLUR_FILTER_WIDTH * BLUR_FILTER_WIDTH, cudaMemcpyHostToDevice));

    blurFilterKernel<< >>(d_blurred_image, d_input_image, d_blur_filter, image_size);
